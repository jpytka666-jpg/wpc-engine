//! WPC v4 (affine 2-bit quantization, two codes per byte) weight backend.
//!
//! A faithful twin of `wpc_weights_v3`, differing in the block type and the
//! file names it opens. What differs in substance is the precision: v4 fits 16
//! levels per 128-weight block where v3 fits 64, so reconstruction is genuinely
//! coarser -- this is not the free re-layout that v2 -> v3 was.
//!
//! What it buys is 68 bytes per 128 weights instead of 100. A 4B model lands
//! near 1.9 GiB instead of 2.9 GiB, which is the difference between fitting a
//! 4 GB card and missing it by ~100 MB. Generation here is memory-bandwidth
//! bound, so the smaller read is also less time per token.

use crate::weights::{EmbeddingTable, Linear};
use memmap2::Mmap;
use serde::Deserialize;
use std::collections::HashMap;
use std::fs::File;
use std::path::Path;
use std::sync::Arc;
use wpc_format::{QuantBlockV5, BLOCK_SIZE_V5};

#[derive(Debug, Deserialize)]
struct LayerMetaV5 {
    name: String,
    #[allow(dead_code)]
    shape: Vec<usize>,
    offset_bytes: usize,
    size_bytes: usize,
}

#[derive(Debug, Deserialize)]
struct ModelMetaV5 {
    layers: Vec<LayerMetaV5>,
    block_size: usize,
}

/// Shared, load-once WPC v4 model data: mmap'd concatenated packed blocks for
/// every tensor. No dictionaries.
pub struct WpcModelDataV5 {
    mmap: Mmap, // model_v5.wpc: all tensors' QuantBlockV5 runs, concatenated
    offsets: HashMap<String, (usize, usize)>, // name -> (offset_bytes, size_bytes)
    has_avx2_fma: bool,
}

impl WpcModelDataV5 {
    pub fn open(wpc_dir: &Path) -> anyhow::Result<Arc<WpcModelDataV5>> {
        let meta_path = wpc_dir.join("model_v5.meta");
        let meta_text = std::fs::read_to_string(&meta_path)?;
        let meta: ModelMetaV5 = serde_json::from_str(&meta_text)?;

        anyhow::ensure!(
            meta.block_size == BLOCK_SIZE_V5,
            "model_v5.meta block_size {} doesn't match BLOCK_SIZE_V5 {}",
            meta.block_size,
            BLOCK_SIZE_V5
        );

        let wpc_path = wpc_dir.join("model_v5.wpc");
        let file = File::open(&wpc_path)?;
        let mmap = unsafe { Mmap::map(&file)? };

        let mut offsets = HashMap::with_capacity(meta.layers.len());
        for l in &meta.layers {
            offsets.insert(l.name.clone(), (l.offset_bytes, l.size_bytes));
        }

        let has_avx2_fma = is_x86_feature_detected!("avx2") && is_x86_feature_detected!("fma");
        if !has_avx2_fma {
            eprintln!("[wpc_weights_v5] CPU lacks AVX2+FMA; falling back to scalar v4 decode (slower).");
        }

        Ok(Arc::new(WpcModelDataV5 {
            mmap,
            offsets,
            has_avx2_fma,
        }))
    }

    /// Byte range within `model_v5.wpc` for tensor `name`.
    fn tensor_range(&self, name: &str) -> (usize, usize) {
        self.offsets
            .get(name)
            .map(|(offset, size)| (*offset, *size))
            .unwrap_or_else(|| panic!("tensor {name} not found in model_v5.meta"))
    }

    /// Reinterpret the tensor's byte range as a `&[QuantBlockV5]`.
    ///
    /// Sound because `QuantBlockV5` is `repr(C, packed)` of `f16, f16, [u8;64]`
    /// -- alignment 1, no padding, no invalid bit patterns -- so any 68-byte
    /// run is a valid instance.
    fn blocks_for(&self, name: &str) -> &[QuantBlockV5] {
        let (offset, size) = self.tensor_range(name);
        let bytes = &self.mmap[offset..offset + size];
        assert_eq!(bytes.len() % QuantBlockV5::SIZE, 0, "misaligned v4 block range");
        let n_blocks = bytes.len() / QuantBlockV5::SIZE;
        unsafe { std::slice::from_raw_parts(bytes.as_ptr() as *const QuantBlockV5, n_blocks) }
    }
}

/// Decode one row's blocks into `out` (plain weights, not a dot product).
/// Used by `WpcEmbeddingV5::embed` for token lookups.
fn row_decode_into_v5(blocks: &[QuantBlockV5], out: &mut [f32]) {
    let mut out_idx = 0;
    for b in blocks {
        let zp = b.zero_point.to_f32();
        let scale = b.scale.to_f32();
        let packed = b.packed;
        let codes = QuantBlockV5::unpack_codes(&packed);
        for &code in codes.iter() {
            out[out_idx] = zp + code as f32 * scale;
            out_idx += 1;
        }
    }
}

/// A WPC v4-compressed `Linear` layer.
pub struct WpcLinearV5 {
    data: Arc<WpcModelDataV5>,
    tensor_name: String,
    out_features: usize,
    in_features: usize,
    bias: Option<Vec<f32>>,
}

impl WpcLinearV5 {
    pub fn new(
        data: Arc<WpcModelDataV5>,
        tensor_name: &str,
        out_features: usize,
        in_features: usize,
        bias: Option<Vec<f32>>,
    ) -> Self {
        assert_eq!(
            in_features % BLOCK_SIZE_V5,
            0,
            "in_features must be a multiple of {BLOCK_SIZE_V5} for WPC v4 blocks"
        );
        let (_, size) = data.tensor_range(tensor_name);
        let expected_blocks = out_features * (in_features / BLOCK_SIZE_V5);
        assert_eq!(
            size / QuantBlockV5::SIZE,
            expected_blocks,
            "tensor {tensor_name}: v4 block count mismatch (meta says {}, shape implies {})",
            size / QuantBlockV5::SIZE,
            expected_blocks
        );
        WpcLinearV5 {
            data,
            tensor_name: tensor_name.to_string(),
            out_features,
            in_features,
            bias,
        }
    }
}

impl Linear for WpcLinearV5 {
    fn in_features(&self) -> usize {
        self.in_features
    }
    fn out_features(&self) -> usize {
        self.out_features
    }
    fn matvec(&self, x: &[f32], y: &mut [f32]) {
        debug_assert_eq!(x.len(), self.in_features);
        debug_assert_eq!(y.len(), self.out_features);
        let blocks_per_row = self.in_features / BLOCK_SIZE_V5;
        let blocks = self.data.blocks_for(&self.tensor_name);
        let has_avx2 = self.data.has_avx2_fma;

        use rayon::prelude::*;
        y.par_iter_mut()
            .zip(blocks.par_chunks_exact(blocks_per_row))
            .for_each(|(o, row_blocks)| {
                if has_avx2 {
                    *o = unsafe {
                        wpc_eval::fused_kernel::matvec_v5_fused(
                            row_blocks,
                            x.as_ptr(),
                            row_blocks.len(),
                        )
                    }
                } else {
                    *o = wpc_eval::fused_kernel::matvec_v5_scalar(row_blocks, x);
                }
            });
        if let Some(b) = &self.bias {
            for (o, bi) in y.iter_mut().zip(b.iter()) {
                *o += *bi;
            }
        }
    }
}

/// WPC v4-compressed embedding / tied lm_head table: `[vocab_size, hidden_size]`.
pub struct WpcEmbeddingV5 {
    data: Arc<WpcModelDataV5>,
    tensor_name: String,
    vocab_size: usize,
    hidden_size: usize,
}

impl WpcEmbeddingV5 {
    pub fn new(
        data: Arc<WpcModelDataV5>,
        tensor_name: &str,
        vocab_size: usize,
        hidden_size: usize,
    ) -> Self {
        assert_eq!(
            hidden_size % BLOCK_SIZE_V5,
            0,
            "hidden_size must be a multiple of {BLOCK_SIZE_V5} for WPC v4 blocks"
        );
        let (_, size) = data.tensor_range(tensor_name);
        let expected_blocks = vocab_size * (hidden_size / BLOCK_SIZE_V5);
        assert_eq!(
            size / QuantBlockV5::SIZE,
            expected_blocks,
            "tensor {tensor_name}: v4 block count mismatch (meta says {}, shape implies {})",
            size / QuantBlockV5::SIZE,
            expected_blocks
        );
        WpcEmbeddingV5 {
            data,
            tensor_name: tensor_name.to_string(),
            vocab_size,
            hidden_size,
        }
    }
}

impl EmbeddingTable for WpcEmbeddingV5 {
    fn vocab_size(&self) -> usize {
        self.vocab_size
    }
    fn hidden_size(&self) -> usize {
        self.hidden_size
    }
    fn embed(&self, token_id: u32, out: &mut [f32]) {
        debug_assert_eq!(out.len(), self.hidden_size);
        let blocks_per_row = self.hidden_size / BLOCK_SIZE_V5;
        let all_blocks = self.data.blocks_for(&self.tensor_name);

        let row_start = token_id as usize * blocks_per_row;
        let row_blocks = &all_blocks[row_start..row_start + blocks_per_row];
        row_decode_into_v5(row_blocks, out);
    }
    fn logits(&self, x: &[f32], out: &mut [f32]) {
        debug_assert_eq!(x.len(), self.hidden_size);
        debug_assert_eq!(out.len(), self.vocab_size);
        let blocks_per_row = self.hidden_size / BLOCK_SIZE_V5;
        let blocks = self.data.blocks_for(&self.tensor_name);
        let has_avx2 = self.data.has_avx2_fma;

        use rayon::prelude::*;
        out.par_iter_mut()
            .zip(blocks.par_chunks_exact(blocks_per_row))
            .for_each(|(o, row_blocks)| {
                if has_avx2 {
                    *o = unsafe {
                        wpc_eval::fused_kernel::matvec_v5_fused(
                            row_blocks,
                            x.as_ptr(),
                            row_blocks.len(),
                        )
                    }
                } else {
                    *o = wpc_eval::fused_kernel::matvec_v5_scalar(row_blocks, x);
                }
            });
    }
}
