...
impl Mcp {
    fn spawn(cmd: &str, args: &[String]) -> Result<Self> {
        ...
        c.request("initialize", json!({
            ...
            "capabilities": {},
            "clientInfo": {"name": "wpc-aions-agent", "version": env!("CARGO_PKG_VERSION")}
        }))?;
    ...
    fn send(&mut self, v: &Value) -> Result<()> {
        serde_json::to_writer(&mut self.input, v)?;
    ...
    fn tools(&mut self) -> Result<Vec<Value>> {
        Ok(self.request("tools/list", json!({}))?
            .get("tools")
...
///
/// The catalogue is inserted into every model turn. On the target hardware,
/// keeping the prompt compact is essential, so we expose the tool name, its
/// first description line, and the argument names. The live MCP server remains
/// the authority for the full schema when a call is executed.
...
fn action(s: &str) -> Result<Action> {
    for line in s.lines().map(str::trim) {
        if let Some(x) = line.strip_prefix("TOOL_CALL ") {
...
fn model(args: &Args, p: &str) -> Result<String> {
    let o = Command::new(&args.runtime)
        .args(["--model"])
        .arg(&args.model)
        .args(["--wpc"])
        .arg(&args.wpc)
        .args(["--scheme", &args.scheme, "--arch", &args.arch, "--prompt", p, "--max-tokens"])
        .arg(args.max_tokens.to_string())
        .output()?;
    if !o.status.success() {
        bail!("wpc-runtime failed: {}", String::from_utf8_lossy(&o.stderr))
    }
...
fn main() -> Result<()> {
    ...
    let cmd = a
        .mcp_command
        .clone()
        .or_else(|| std::env::var("AIONS_MCP_COMMAND").ok())
        .ok_or_else(|| anyhow!("set --mcp-command or AIONS_MCP_COMMAND"))?;

    let mut m = Mcp::spawn(&cmd, &a.mcp_arg)?;
    let all = m.tools()?;
    let selected = narrow(all.clone(), &a.tools);
    eprintln!(
        "AIONS MCP: {} tools available, {} exposed to the model",
        all.len(),
        selected.len()
    );

    let tools = manifest(&selected);
    let mut transcript = format!("TASK: {}\n", a.task);

    for turn in 1..=a.max_turns {
        eprintln!("=== AIONS AGENT TURN {turn}/{} ===", a.max_turns);
        let r = model(&a, &prompt(&tools, &transcript))?;
        eprintln!("MODEL: {r}");
