...
struct Args {
    #[arg(long)]
    task: String,
    #[arg(long, default_value = "/home/aions/qwen3-coder-run")]
    model: PathBuf,
    #[arg(long, default_value = "/home/aions/qwen3-coder-wpc4")]
    wpc: PathBuf,
    #[arg(long, default_value = "v4")]
    scheme: String,
    #[arg(long, default_value = "qwen3-moe")]
    arch: String,
...
fn narrow(tools: Vec<Value>, wanted: &[String]) -> Vec<Value> {
    ...
    tools
        ...
        .filter(|t| {
            t.get("name")
                .and_then(Value::as_str)
                .map(|n| wanted.iter().any(|w| n.contains(w)))
                .unwrap_or(false)
        })
        .collect()
}
...
fn main() -> Result<()> {
    ...
    eprintln!(
        "loading WPC model once; it will remain resident across {} agent turns",
        a.max_turns
    );
