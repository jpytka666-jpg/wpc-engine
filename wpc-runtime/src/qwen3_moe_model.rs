...
impl MoeKvCache {
    ...
    pub fn restore_from_files(
        ...
        logits_path: impl AsRef<Path>,
    ) -> anyhow::Result<Vec<f32>> {
        let snapshot = crate::kv_probe::KvSnapshot::read_from_path(snapshot_path)?;
        anyhow::ensure!(
            !snapshot.truncated,
            "KV snapshot is truncated and cannot be restored"
        );
        anyhow::ensure!(
            !snapshot.records.is_empty(),
            "KV snapshot contains no records"
        );
        let num_layers = self.layers.len();
        let num_kv_heads = self.layers.first().map(|l| l.k.len()).unwrap_or(0);
        let per_position = num_layers
            .checked_mul(num_kv_heads)
            .ok_or_else(|| anyhow::anyhow!("KV restore count overflow"))?;
        let sequence_length = snapshot
            .records
            .iter()
            .map(|r| r.position as usize + 1)
            .max()
            .unwrap_or(0);
        anyhow::ensure!(sequence_length > 0, "KV snapshot has zero sequence length");
        anyhow::ensure!(
            snapshot.records.len() == per_position * sequence_length,
            "KV snapshot record count does not match runtime geometry"
        );
        for layer in &mut self.layers {
            for head in &mut layer.k {
                head.clear();
                head.resize(sequence_length * self.head_dim, 0.0);
            }
            for head in &mut layer.v {
                head.clear();
                head.resize(sequence_length * self.head_dim, 0.0);
        ...
        for record in snapshot.records {
            let layer = record.layer as usize;
            let head = record.kv_head as usize;
            ...
            anyhow::ensure!(
                ...
                "KV snapshot position out of range"
            );
            anyhow::ensure!(
                record.key.len() == self.head_dim && record.value.len() == self.head_dim,
                "KV snapshot dimension mismatch"
            );
            anyhow::ensure!(
                seen.insert((layer, head, position)),
                "duplicate KV snapshot record"
            );
        ...
        anyhow::ensure!(
            seen.len() == per_position * sequence_length,
            "KV snapshot has missing records"
        );
        ...
        anyhow::ensure!(
            ...
            "logits sidecar vocabulary size mismatch"
        );
        Ok(logits)
    }
}
