...
fn build() -> (WeightSetManager, ExternalTransformer, WeightSetId, Vec<TensorSnapshot>) {
    ...
    let manifest=WeightSetManifest::new(WeightSetHeader::new(WeightSetId::new("observatory-train-v1"),WeightSetVersion::new("0.1.0").unwrap(),ArchitectureId::new("noworodek-decoder-v0")),specs).unwrap();
...
fn forward_loss(model:&ExternalTransformer, manager:&WeightSetManager, tokens:&[usize], target_token:usize)->(f32,Vec<f32>){
...
fn main(){
    ...
    let (loss0,_)=forward_loss(&model,&manager,&tokens,target);
    println!("NOWORODEK OBSERVATORY TRAIN+INFLUENCE V1");
    println!("experience_id=math.next_token.v1 target={} steps={} lr={}",target,steps,lr);
    println!("BEFORE loss={:.9}",loss0);
    ...
    for step in 0..steps {
        ...
        for hidden_idx in 0..model.config.hidden_size { let idx=target*model.config.hidden_size+hidden_idx; w[idx]+=lr; }
        h.write(&mut manager,&Tensor::from_vec(vec![model.config.vocab_size,model.config.hidden_size],w).unwrap()).unwrap();
        let (after_loss,after_out)=forward_loss(&model,&manager,&tokens,target);
        let max_delta=before.iter().zip(after_out.iter()).map(|(a,b)|(*b-*a).abs()).fold(0.0f32,f32::max);
        if step==0 || step%100==99 { println!("step={} loss={:.9} delta_logits_max={:.9}",step+1,after_loss,max_delta); }
        trace.push((step+1,after_loss,max_delta));
    }

    let (loss1,_)=forward_loss(&model,&manager,&tokens,target);
    ...
    println!("AFTER loss={:.9} loss_reduction={:.9}",loss1,loss0-loss1);
    println!("OBSERVATIONS={}",trace.len());
    println!("RESULT experience_to_weight_delta=true");
}
