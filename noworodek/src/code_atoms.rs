...
impl fmt::Display for CodeAtomId {
...
impl CodeAtom {
    ...
    pub fn with_experience(mut self, experience_id: impl Into<String>) -> Self {
        self.experience_id = Some(experience_id.into());
        self.id = make_id(self.language, self.kind, &self.source, &self.version, self.parent.as_ref(), self.related.as_ref());
        self
    }

    pub fn with_related(mut self, related: CodeAtomId) -> Self {
        self.related = Some(related);
        self.id = make_id(self.language, self.kind, &self.source, &self.version, self.parent.as_ref(), self.related.as_ref());
        self
    }

    fn build(language: CodeLanguage, kind: CodeAtomKind, source: String, version: String, parent: Option<CodeAtomId>, related: Option<CodeAtomId>, experience_id: Option<String>) -> Self {
        let id = make_id(language, kind, &source, &version, parent.as_ref(), related.as_ref());
        Self { id, language, kind, source, version, parent, related, experience_id }
    }
...
impl CodeAtomRegistry {
    ...
    pub fn insert(&mut self, atom: CodeAtom) -> Result<bool, String> {
        if let Some(existing) = self.atoms.get(&atom.id) {
            if existing != &atom { return Err("atom id collision with different payload".into()); }
            return Ok(false);
        }
    ...
    pub fn contains(&self, id: &CodeAtomId) -> bool { self.atoms.contains_key(id) }
    pub fn parent_of(&self, id: &CodeAtomId) -> Option<CodeAtomId> { self.get(id).and_then(|a| a.parent().cloned()) }
    pub fn children_of(&self, parent: &CodeAtomId) -> Vec<&CodeAtom> {
        self.atoms.values().filter(|a| a.parent() == Some(parent)).collect()
    }
    pub fn by_language_kind(&self, language: CodeLanguage, kind: CodeAtomKind) -> Vec<&CodeAtom> {
        self.atoms.values().filter(|a| a.language == language && a.kind == kind).collect()
    }
}
...
fn make_id(language: CodeLanguage, kind: CodeAtomKind, source: &str, version: &str, parent: Option<&CodeAtomId>, related: Option<&CodeAtomId>) -> CodeAtomId {
    ...
    feed(source); feed(version);
    feed(parent.map(CodeAtomId::as_str).unwrap_or(""));
    feed(related.map(CodeAtomId::as_str).unwrap_or(""));
...
mod tests {
    use super::*;
    #[test] fn stable_id_for_same_payload() { assert_eq!(CodeAtom::new(CodeLanguage::Rust, CodeAtomKind::Function, "fn x(){}", "1").id(), CodeAtom::new(CodeLanguage::Rust, CodeAtomKind::Function, "fn x(){}", "1").id()); }
    #[test] fn outer_whitespace_is_canonicalized() { assert_eq!(CodeAtom::new(CodeLanguage::Rust, CodeAtomKind::Function, "  fn x(){}  ", "1").id(), CodeAtom::new(CodeLanguage::Rust, CodeAtomKind::Function, "fn x(){}", "1").id()); }
}
