...
impl CodeAtomRegistry {
    ...
    pub fn children_of(&self, parent: &CodeAtomId) -> Vec<&CodeAtom> { self.atoms.values().filter(|a| a.parent() == Some(parent)).collect() }
    pub fn by_language_kind(&self, language: CodeLanguage, kind: CodeAtomKind) -> Vec<&CodeAtom> { self.atoms.values().filter(|a| a.language == language && a.kind == kind).collect() }
}
