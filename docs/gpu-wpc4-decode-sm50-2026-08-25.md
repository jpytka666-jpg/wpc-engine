...
## Step 3 — fused decode and multiply, 2026-08-25 15:22

The chunking constraint above said the expanded form must never be materialised. The
fused kernel `wpc4_gemv` acts on that: it decodes each weight **inside** the multiply, so
the weight exists only in a register. Input is the packed model in VRAM plus an
activation vector; output is the result vector. Nothing else is written.

The activation vector is generated by a fixed LCG, so the run is reproducible.

### Correctness — and why it is not bit-exact here

Unlike the decode gate, this one cannot demand bit-for-bit equality and claiming it would
be false: the GPU sums the products in a tree, the host sums them in sequence. Both are
single precision, so both are approximations of the same exact sum. The honest test is a
**double-precision reference** that both single-precision paths are measured against.

| Tensor | Shape | GPU vs double | Host float vs double | Verdict |
|---|---|---|---|---|
| `layers.12.self_attn.o_proj` | 2560 × 4096 | **2.235e-04** | 1.592e-03 | PASS |
| `layers.1.mlp.down_proj` | 2560 × 9728 | **2.065e-04** | 8.588e-04 | PASS |
| `embed_tokens` | 151936 × 2560 | **2.695e-02** | 1.752e-01 | PASS |

**The GPU is closer to the exact answer than the host float path in every case** — by 4x
to 8x. That is expected: a tree reduction accumulates less rounding error than a long
sequential sum. The kernel is not merely acceptable, it is the more accurate of the two.

The embedding row shows larger relative error on both paths because random activations
against embedding weights produce heavily cancelling sums, where a relative measure
magnifies small absolute differences. The GPU/host *ratio* is what matters, and it holds.

### Speed and traffic

| Tensor | Packed | Fused kernel | Weights/s | Packed traffic | f32 traffic for same work |
|---|---|---|---|---|---|
| `o_proj` | 5.3 MiB | 1.043 ms | 10 049.7 M/s | 5.34 GB/s | 40.20 GB/s |
| `down_proj` | 12.6 MiB | 2.132 ms | **11 683.5 M/s** | 6.21 GB/s | 46.73 GB/s |
| `embed_tokens` | 197.1 MiB | 44.532 ms | 8 734.4 M/s | 4.64 GB/s | 34.94 GB/s |

15x to 19x faster than the scalar host float path. The right-hand column is the point of
keeping WPC on the card: the same arithmetic against f32 weights would demand 7.5x the
memory bandwidth, and bandwidth is what this card is short of.

### The constraint is gone

`embed_tokens` is the tensor that **could not be expanded at all** — 1483.8 MiB of f32
against 3410 MiB free with a 2037.8 MiB model already resident. Fused, it computes in
44.5 ms and returns a 594 KiB result vector. Nothing is written back but the answer.
