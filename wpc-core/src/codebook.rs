...
impl ResidualDict {
    pub fn train(data: &[[f32; BLOCK_DIM]], k: usize, iters: usize) -> Self {
        // Subsample to 16x k so k-means actually clusters (Lloyd's algorithm needs n
        // significantly larger than k to do meaningful work) while guaranteeing n >= k
        // so kmeans() never falls into its n<=k duplicate-padding fast path.
        let max_samples = k.saturating_mul(16);
